...
def main():
	...
	# colores
	temasColores = ["Greens", "PuBu", "coolwarm", "viridis", "inferno"]
	...
	# save info
	rutaCarpetaParaGuardarCSV = "{}/estaciones/{}-{:02d}-{:02d}/{}-{:02d}-{:02d}.csv".format(RUTA_CARPETA, anio, mes, dia,anio, mes, dia)
	data.to_csv(rutaCarpetaParaGuardarCSV)
	...
	for t in tiempos:
		...
		for columna in arrayColumns:
			...
			# limites min y max
			LONG_MIN = x_values[0] - 0.2
			LONG_MAX = x_values[-1] + 0.2

			LAT_MIN = y_values[0] - 0.2
			LAT_MAX = y_values[-1] + 0.2
			...
			print("cols, rows", numCols, numRows)
			# generar xi, yi
			...
			def generarClevs(z):
				...
				print("***** MIN Z", z.min())
				if step <= 0.1:
					return np.linspace(0.0,1,10)
				else:
					return np.linspace(0, z.max(), 10)
			
			clevs = generarClevs(z)
			...
			cs = m.contourf(xi,yi,zi, clevs, zorder=5, alpha=0.5, cmap="hot_r")
			
			# agregar archivo shape de estados
			m.readshapefile('../shapes/Estados', 'Estados')

			# agregar puntos de estaciones
			m.scatter(xC, yC, latlon=True,s=1, color="r", marker='o', zorder=25)
			...
			print("***** PROCESAMIENTO PARA: ", columna)
			nombreTemporalParaElMapa = "{}/estaciones/maps/{}-{:02d}-{:02d}/{}_{}.png".format(RUTA_CARPETA, anio, mes, dia, t,columna)
